...
def expand_indices(counts):
    ...
    for index, count in enumerate(counts):
        if count > 0:
            expanded_indices.extend([index] * count)
    return np.array(expanded_indices)

def get_max_batch(rad2, m ,n ):
    mmax = 0
    for j in range(m):
        radj = rad2[j*n : (j+1)*n] 
        idxj = expand_indices(radj)
        if len(idxj) > mmax:
            mmax = len(idxj)
    return mmax 

def DEAREST(num_iters, comm,  x0, i, m, n, kappa, K, print_freq, fi, gradient, full_gradient, lr, B, p, fname):
    ...
    for epoch in tqdm(range(num_iters)):
        ...
        if epoch == 0:
            ...
            if rad1 < p:
                ...
                rad2 = np.random.multinomial(B, [1 / (m*n)] * m*n)
                radi = rad2[i*n : (i+1)*n] 
                idx = expand_indices(radi)
                full_idx = expand_indices(rad2)
                grad_cnt += 2 * len(full_idx)
                comp_cnt += 2 * get_max_batch(rad2, m , n)
                if len(idx) > 0:
